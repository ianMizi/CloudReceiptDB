/****** Object:  Function [dbo].[FlatBill_CurrentOutStandingBalance]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE FUNCTION [dbo].[FlatBill_CurrentOutStandingBalance](@InvoiceNo varchar(100),@EnterpriseAccNo varchar(100))
RETURNS varchar (300)
AS
BEGIN
  
	    DECLARE @CurrentOutStandingBalance as varchar (300)
		   --DECLARE @Result as varchar
    /*
        SELECT dbo.f_BinaryToBase64(CONVERT(VARBINARY(MAX), 'Converting this text to Base64...'))
    */

     SET @CurrentOutStandingBalance = (select top 1 FlatBillOutStandingBalance  from FlatBill where FlatBillInvoiceNo=@InvoiceNo and CloudreceiptAccNo=@EnterpriseAccNo order by ID desc)
	 if @CurrentOutStandingBalance is null
	  SET @CurrentOutStandingBalance ='0.00'
	  else
	   SET @CurrentOutStandingBalance =@CurrentOutStandingBalance
