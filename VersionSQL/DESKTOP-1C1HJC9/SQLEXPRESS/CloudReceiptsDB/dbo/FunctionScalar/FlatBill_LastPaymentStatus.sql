/****** Object:  Function [dbo].[FlatBill_LastPaymentStatus]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE FUNCTION [dbo].[FlatBill_LastPaymentStatus](@InvoiceNo varchar(100),@EnterpriseAccNo varchar(100))
RETURNS varchar  (300)
AS
BEGIN
  
	    DECLARE @LastPaymentStatus as varchar (300)
		   --DECLARE @Result as varchar
    /*
        SELECT dbo.f_BinaryToBase64(CONVERT(VARBINARY(MAX), 'Converting this text to Base64...'))
    */

     SET @LastPaymentStatus = (select top 1 FlatBillTaxInvoice  from FlatBill where FlatBillInvoiceNo=@InvoiceNo and CloudreceiptAccNo=@EnterpriseAccNo order by ID desc)
	 if @LastPaymentStatus is null
	  SET @LastPaymentStatus ='Unpaid'
	  else
	   SET @LastPaymentStatus =@LastPaymentStatus
