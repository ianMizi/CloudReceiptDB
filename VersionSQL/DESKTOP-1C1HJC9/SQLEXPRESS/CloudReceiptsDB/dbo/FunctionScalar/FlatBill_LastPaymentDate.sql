/****** Object:  Function [dbo].[FlatBill_LastPaymentDate]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE FUNCTION [dbo].[FlatBill_LastPaymentDate](@InvoiceNo varchar(100),@EnterpriseAccNo varchar(100))
RETURNS varchar (300)
AS
BEGIN
  
	    DECLARE @LastPaymentDate as varchar (300)
		   --DECLARE @Result as varchar
    /*
        SELECT dbo.f_BinaryToBase64(CONVERT(VARBINARY(MAX), 'Converting this text to Base64...'))
    */

     SET @LastPaymentDate = (select top 1 FlatBillImgLogo64  from FlatBill where FlatBillInvoiceNo=@InvoiceNo and CloudreceiptAccNo=@EnterpriseAccNo order by ID desc)
 if @LastPaymentDate is null
	  SET @LastPaymentDate ='NA'
	  else
